# -*- coding: utf-8 -*-
"""
    flask.ext.datastore.bulbs
    ~~~~~~~~~~~~~~

    Defines a Bulbs datastore.

    :copyright: (c) 2012 by Frankie Robertson, 2011 wilsaj.
...
class BulbsDatastore(AdminDatastore):
    """A datastore class for accessing Bulbs models.

    The `models` parameter should be either a module or an iterable
    (like a tuple or a list) that contains the Bulbs models that
    will be made available through the admin interface.

    `graph` should be the Bulbs graph that the datastore
    ...
    XXX: Finally, the `exclude_pks` parameter can be used to specify
    ...
    """
    def __init__(self, models, graph, model_forms=None, exclude_pks=True):
        self.model_classes = {}
        ...
        if type(models) == types.ModuleType:
            self.model_classes = dict(
                ((k, v) for k, v in models.__dict__.items()
                 if isinstance(v, bulbs.model.Model)
                 and k != 'Base'))
        else:
            self.model_classes = dict(
                ((model.__name__, model)
                 for model in models
                 if isinstance(model, bulbs.model.Model)))

        if self.model_classes:
            self.form_dict = dict(
                [(k, _form_for_model(v, graph,
